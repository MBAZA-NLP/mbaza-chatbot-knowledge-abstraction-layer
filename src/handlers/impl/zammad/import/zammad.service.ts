...
import { ZammadAnswerModel } from "../import/zammad.answer-model";
import { ZammadCategoryModel } from "../import/zammad.category-model";
import { ZammadUtils } from "./zammad.utils";
import { ZammadQueryBuilder } from "../import/zammad.query-builder";
    ...
    async getHierarchy(parentID: number, expandTags = true): Promise<ZammadCategoryModel[]> {
        ...
        try {
            ...
            for (const result of results) {
                ...
                const answers = [
                    ...await this.getAnswers(result.id, "en-gb"),
                    ...await this.getAnswers(result.id, IntentLanguages.ENGLISH),
                    ...await this.getAnswers(result.id, IntentLanguages.FRENCH),
                    ...await this.getAnswers(result.id, IntentLanguages.KINYARWANDA)
                ];
